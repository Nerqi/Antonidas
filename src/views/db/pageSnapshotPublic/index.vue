    ...
    <container type="card">
        <template slot="header">
            <p class="warn-content">
                ...
                在 Aadmin 的本页示例中你可以随意填写这个表单，
                表单内容会自动实时持久化，
                无论是切换标签页、重新打开标签页、刷新浏览器、重开浏览器、重开浏览器标签页等，
                该页面数据都会自动恢复到上次填写的状态，
                这些都只需要你使用 Aadmin 提供的两个方法，
                总共只需要多写十几行代码
            </p>
        </template>
        ...
        <el-button
            ...
            type="danger"
            @click="handleClear">
            <svg-icon iconClass="plane"></svg-icon> 删除当前页面快照
        </el-button>
    </container>
</template>
    ...
    export default {
        ...
        async created () {
            ...
            for (const key in data) {
                console.log(key)
                if (data.hasOwnProperty(key)) {this[key] = data[key]; console.log(this[key])}
            }
        },
        watch: {
            $data: {
